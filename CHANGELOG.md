...
* Changed `-st2-auth` secret so that it can be loaded via `envFrom` instead of listing `ST2_AUTH_USERNAME` and `ST2_AUTH_PASSWORD` separately for each container that needs them. (#257) (by @cognifloyd)
